policy "1" {
  ...
  check "1.1" {
    title = "AWS CIS 1.1 Avoid the use of 'root' account. Show used in last 30 days (Scored)"
    doc = file("cis_v1.2.0/docs/1.1.md")
    query       = file("queries/iam/avoid_root_usage.sql")
  }

  check "1.2" {
    title = "AWS CIS 1.2 Ensure MFA is enabled for all IAM users that have a console password (Scored)"
    doc = file("cis_v1.2.0/docs/1.2.md")
    query       = file("queries/iam/mfa_enabled_for_console_access.sql")
  }

  check "1.3" {
    title = "AWS CIS 1.3 Ensure credentials unused for 90 days or greater are disabled (Scored)"
  ...
  check "1.5" {
    title = "AWS CIS 1.5  Ensure IAM password policy requires at least one uppercase letter"
    doc = file("cis_v1.2.0/docs/1.5.md")
    query       = file("queries/iam/password_policy_min_uppercase.sql")
  }

  check "1.6" {
    title = "AWS CIS 1.6  Ensure IAM password policy requires at least one lowercase letter"
  ...
  check "1.8" {
    title = "AWS CIS 1.8  Ensure IAM password policy requires at least one number"
    doc = file("cis_v1.2.0/docs/1.8.md")
    query       = file("queries/iam/password_policy_min_number.sql")
  }

  check "1.9" {
    title = "AWS CIS 1.9 Ensure IAM password policy requires minimum length of 14 or greater"
  ...
  check "1.11" {
    title = "AWS CIS 1.11 Ensure IAM password policy expires passwords within 90 days or less"
  ...
  check "1.13" {
    title = "AWS CIS 1.13 Ensure MFA is enabled for the 'root' account"
    doc = file("cis_v1.2.0/docs/1.13.md")
    query       = file("queries/iam/mfa_enabled_for_root.sql")
  }

  check "1.14" {
    title = "AWS CIS 1.14 Ensure hardware MFA is enabled for the 'root' account (Scored)"
    doc = file("cis_v1.2.0/docs/1.14.md")
    query       = file("queries/iam/hardware_mfa_enabled_for_root.sql")
  }

  check "1.16" {
    title = "AWS CIS 1.16 Ensure IAM policies are attached only to groups or roles (Scored)"
