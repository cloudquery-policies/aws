policy "2" {
  ...
  check "2.1" {
    title = "AWS CIS 2.1 Ensure CloudTrail is enabled in all regions"
  ...
  check "2.4" {
    title = "AWS CIS 2.4 Ensure CloudTrail trails are integrated with CloudWatch Logs"
    doc   = file("cis_v1.2.0/docs/2.4.md")
    query = file("queries/cloudtrail/integrated_with_cloudwatch_logs.sql")
  }

  check "2.6" {
    title = "AWS CIS 2.6 Ensure S3 bucket access logging is enabled on the CloudTrail S3 bucket"
    doc   = file("cis_v1.2.0/docs/2.6.md")
    query = file("queries/cloudtrail/bucket_access_logging.sql")
  }

  check "2.7" {
    title = "AWS CIS 2.7 Ensure CloudTrail logs are encrypted at rest using KMS CMKs"
    doc   = file("cis_v1.2.0/docs/2.7.md")
    query = file("queries/cloudtrail/logs_encrypted.sql")
  }

  check "2.8" {
    title = "AWS CIS 2.8 Ensure rotation for customer created CMKs is enabled (Scored)"
