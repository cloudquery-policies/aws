policy "other" {
  ...
  check "ec2-ebs-snapshots-shared-externally" {
    type  = "manual"
    title = "Find all ec2 ebs snapshots that are shared with external accounts."
    query = file("queries/ec2/ebs_snapshot_shared_external_acc.sql")
  }
